...
public class TouchCollector {
	 ...
	 * 
	 * Information: ASM version 4.1 does not allow for the data type
	 * java.lang.Class to be a parameter to the method visitLdcInsn which causes
	 * issues for anything below .class versions 49 and lower. Changing the
	 * registered class to use instead a String parameter and search for the
	 * class in the classpath helped resolve the issue. Also as a side note: The
	 * replace parameters might enter as "java/lang/String" and need to be
	 * translated to "java.lang.String" so the forName method can understand it.
	 * 
	 * @param classa
	...
	public static synchronized void registerClass(String classa)
			throws ClassNotFoundException {
		// If it's not in the system jvm, then search the current thread for
		// the class.
		// This is a dirty hack to guarantee that multiple classloaders can
		// invoke cobertura code.

		// We try 2 methods to register the classes
		// First method we try to call the invoker classloader. If the
		// invoker causes an exception (NoClassDefFound) it
		// will then call Thread.currentThread.getContextClassLoader() which
		// gets the current threads classloader and
		// checks to see if cobertura code is in there. This is here because
		// there are situations where multiple
		// classloaders might be invoked and it requires the check of
		...
		try {
			...
			found = registerClassIfInstrumented(clazz);

		} catch (ClassNotFoundException e) {
			logger
					.log(
							Level.FINE,
							"Exception when registering class: Thread.currentThread().getContextClassLoader()"
									+ classa);
		}
		if (!found) {
			try {
				registerClassIfInstrumented(Class.forName(classa.replace("/",
						".")));
			} catch (ClassNotFoundException e) {
	...
	private static boolean registerClassIfInstrumented(final Class<?> clazz) {
		boolean found = false;
